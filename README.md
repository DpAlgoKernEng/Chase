...
| 功能 | 实现位置 | 状态 | 备注 |
|------|----------|------|------|
| Keep-Alive | server.c/h | ✅ | 默认启用，超时配置支持 |
| Chunked 编码 | http_parser.c/h | ✅ | 解析 + 生成完整支持 |
| Host Header 验证 | server.c/h | ✅ | HTTP/1.1 合规验证 |
| Range 请求 | handler.c/h, fileserve.c/h | ✅ | 206/416 状态码支持 |
...
| vhost | vhost.c/h | 虚拟主机匹配（精确 + 通配符） | ✅ |
| config | config.c/h | JSON 配置加载 | ✅ |
...
├── test/              # 测试代码
│   ├── integration/c_core/ # 集成测试 (22 个测试文件)
│   ├── certs/             # SSL 测试证书
│   └── report/             # 测试报告
├── docs/              # 文档
│   ├── superpowers/
